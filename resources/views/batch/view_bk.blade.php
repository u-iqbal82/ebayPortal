                        ...
                        @if($category !== 'all')
                            ...
                            @endphp
                                <tr>
                                    ...
                                    <td>
                                    @if ((($batch->status == 'FullyAssigned') && ($article->status == 'Saved' || $article->status == 'Assigned')) || ($batch->status = 'Submitted' && $article->status == 'Completed'))
                                        <a class="btn btn-sm btn-success" href="/article/view/{{ $article->id }}" role="button">Open Article</a>
                                    @else
                                        <a class="btn btn-sm btn-success disabled" href="/article/view/{{ $article->id }}" disbaled="disabled" role="button">OOpen Article</a>
                                    @endif
                                    ...
                                    @php 
                                    if ($article->status == 'Completed' && Auth::user()->hasPermission(['moved-to-saved'])) {
                                    @endphp
                                    <a class="btn btn-sm btn-info" href="/article/view/{{ $article->id }}/saved" role="button">Move to Saved</a>
                                    @php
                                    }
                                    @endphp
                                    </td>    
                            ...
                            @endphp
                                <tr>
                                    ...
                                    @permission('move-to-saved')
                                        ...
                                        if ($article->status == 'Completed' && Auth::user()->hasPermission(['moved-to-saved'])) {
                                        @endphp
                                        <a class="btn btn-sm btn-info" href="/article/{{ $article->id }}/saved" role="button">Move to Saved</a>
                                        @php
                                        }
                                        @endphp
                                    @endpermission
